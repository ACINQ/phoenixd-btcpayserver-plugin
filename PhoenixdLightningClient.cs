...
namespace BTCPayServer.Lightning.Phoenixd
{
    public class PhoenixdLightningClient : ILightningClient
    {
        ...
        public async Task<LightningInvoice> GetInvoice(string invoiceId, CancellationToken cancellation = default)
        {
            ...
            catch (PhoenixdClient.PhoenixdApiException ex) when (ex.Error.Error == "Not found" || ex.Error.Error.Contains("Invalid hexadecimal", StringComparison.OrdinalIgnoreCase))
            {
                return null;
            }
        }
